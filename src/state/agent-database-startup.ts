...
export async function withAgentDatabaseStartupAdmission<T>(run: () => Promise<T>): Promise<T> {
  if (getAgentDatabaseStartupAdmission()) {
    return run();
  }
  const admission = new AgentDatabaseStartupAdmission();
  try {
    return await startupAdmission.run(admission, run);
  } finally {
